...
module DataMemory(Address, WriteData, Clk, MemWrite, MemRead, ReadData, MemSize,
    ...
    input [5:0] FourRowsIndex; // Which 4 rows to extract from window.

    output reg[31:0] ReadData; // Contents of memory location at Address
    output reg [4*64*8-1:0] FourRows;
    output reg [4*4*8-1:0] Window;
    ...
    always @(*) begin
        for (i = 0; i < 4*4; i = i + 1) begin
            Window[i*8 +: 8] <= memory[4*64*64+i][7:0];
        end
        for (i = 0; i < 4; i = i + 1) begin
            for (j = 0; j < 64; j = j + 1) begin
                FourRows[(i*64+j)*8 +: 8] <= memory[4+((i+FourRowsIndex)*64+j)][7:0];
            end
        end
    end
